...
static void __VirtualProtect(void* address, const int sz)
{
	DWORD old;
	VirtualProtect(address, sz, PAGE_EXECUTE_READWRITE, &old);
}

static void SetHook(BYTE* hook_addr, BYTE* dst, int extra_bytes = 0)
{
	__VirtualProtect(hook_addr, 5 + extra_bytes);

	*hook_addr = 0xE8; // 0xE8 = call
	*(DWORD*)(hook_addr + 1) = dst - (hook_addr + 5);
...
static void ResolvePatterns()
{
	...
	const auto ent_list_ptr = (player_entity***)ResolveAddress(ent_list_pattern, sizeof(ent_list_pattern), 1);

	player_list_ptr = ent_list_ptr;
	player_count = (UINT*)(ent_list_ptr + 2);
	local_player = *(player_entity**)(ent_list_ptr - 1);
	...
	cfg.pFov = (float*)ResolveAddress(fov_pattern, sizeof(fov_pattern), 4);
	cfg.fov = *cfg.pFov;
	...
	BYTE* roll_patch = ResolveAddress(roll_pattern, sizeof(roll_pattern), 0, false);
	...
	BYTE* spread_addr = ResolveAddress(spread_pattern, sizeof(spread_pattern), 0, false);

	SetHook(spread_addr, (BYTE*)SpreadDispatch, 0);
	...
	BYTE* vrecoil_addr = ResolveAddress(vrecoil_pattern, sizeof(vrecoil_pattern), 4, false);

	__VirtualProtect(vrecoil_addr, 4);
	const float* vis_recoil_ptr = &cfg.vis_recoil_mlt;
...
{
	auto target_export = *(DWORD**)((BYTE*)GetProcAddress(hModule, func_name) + 2);
	auto old_func_addr = *(DWORD**)target_export;
...
void initilization(SDL_Window* window)
{
	...
	const auto hModule = GetModuleHandle(L"sdl2.dll");
	SwapWindow = (SwapWindow_sig)HookExport(hModule, "SDL_GL_SwapWindow", (DWORD)DrawMenu);
	PollEvents = (PollEvents_sig)HookExport(hModule, "SDL_PollEvent", (DWORD)HandleEvent);
	
	// Hooking the WndProc
	SDL_SysWMinfo wm_info;
	SDL_VERSION(&wm_info.version);
	SDL_GetWindowWMInfo(window, &wm_info);
	og_wndproc = WndProc_sig(SetWindowLongW(wm_info.info.win.window, GWLP_WNDPROC, (long)WndProc));
